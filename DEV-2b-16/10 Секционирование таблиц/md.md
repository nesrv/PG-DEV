...
## ✅ Бонус: Автоматизация обслуживания (cron + SQL)

* Расписание создания новых партиций
* Удаление устаревших
* `VACUUM`, `ANALYZE`, индексация
* Отчёты по распределению строк

Могу помочь написать такой скрипт (SQL или Bash + `psql`) — просто скажи.
...
| Задача                   | Рекомендации                 |
| ------------------------ | ---------------------------- |
| Индексы                  | Отдельно для каждой партиции |
| Очистка                  | `DROP TABLE` вместо `DELETE` |
| Статистика               | `ANALYZE` по партициям       |
| Добавление партиций      | Скрипт или вручную           |
| Проверка pruning         | `EXPLAIN ANALYZE`            |
| Мониторинг распределения | `pg_class`, `tableoid`       |
